...
function distroyEnemy() {
    for (let i = 0; i < enemies.length; i++) {
        if (enemies[i].x - enemies[i].radius > canvasWidth || enemies[i].y - enemies[i].radius > canvasHeight) {
            enemies.splice(i, 1)
            score -= 5
        }
    }
...
function detectCollisions() {
    ...
    for (let i = 0; i < enemies.length; i++) {
        ...
        for (let j = 0; j < Bullets.length; j++) {
            ...
            const sumRadiiSquared = (enemy.radius + bullet.radius) * (enemy.radius + bullet.radius);
...
document.addEventListener('mousemove', (event) => {
    if (event.clientX > 0 && event.clientX < canvasWidth - player.width)
...
canvas.addEventListener("touchmove", function (event) {
    console.log("moving")
    if (event.touches[0].clientX > 0 && event.touches[0].clientX < canvasWidth - player.width)
...
function getTouchPos(canvasDom, touchEvent) {
  ...
  return (
    <div>
      <canvas id="canvas1" className="bg-gradient-to-tr from-slate-300 to-orange-300"></canvas>
    </div>
  );
};
